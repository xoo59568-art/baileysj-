...
export const makeMessagesRecvSocket = (config) => {
    ...
    // ==================================================
    // Built-in AntiBot — detects bot/link messages in groups
    // (relies on message.isBotMessage / message.isLinkMessage,
    // set in Utils/process-message.js) and enforces
    // kick / warn / delete / null per group, same logic as the
    // old antibot plugin, now native to the socket.
    //
    // IMPORTANT: we listen on the dedicated 'messages.bot-detect' /
    // 'messages.link-detect' events (not 'messages.upsert'). chats.js
    // emits 'messages.upsert' BEFORE processMessage() runs and sets the
    // flags, so reading message.isBotMessage off a 'messages.upsert'
    // listener sees it still undefined. The dedicated events are emitted
    // from inside processMessage() itself, right after the flag is set,
    // so they're always accurate — for messages from anyone, including
    // our own sent messages (detection is not restricted by fromMe).
    // ==================================================
    /** groupJid -> { enabled: boolean, mode: 'kick'|'warn'|'delete'|'null' } */
    const antiBotSettings = new Map();
    /** `${groupJid}:${userJid}` -> warn count, only used in 'warn' mode */
    const antiBotWarns = new Map();
    /** small de-dupe window so a message that is BOTH bot+link isn't double-processed */
    const antiBotSeenIds = new Set();
    const markAntiBotSeen = (id) => {
        if (!id) {
            return false;
        }
        if (antiBotSeenIds.has(id)) {
            return true;
        }
        antiBotSeenIds.add(id);
        if (antiBotSeenIds.size > 500) {
            antiBotSeenIds.delete(antiBotSeenIds.values().next().value);
        }
        return false;
    };
    const getAntiBotSettings = (groupJid) => antiBotSettings.get(groupJid) || { enabled: false, mode: 'kick' };
    const setAntiBotSettings = (groupJid, { enabled, mode } = {}) => {
        const current = getAntiBotSettings(groupJid);
        const next = {
            enabled: typeof enabled === 'boolean' ? enabled : current.enabled,
            mode: mode || current.mode
        };
        antiBotSettings.set(groupJid, next);
        return next;
    };
    const handleAntiBotDetection = async (message) => {
        try {
            const groupJid = message.key?.remoteJid;
            if (!groupJid || !isJidGroup(groupJid)) {
                return; // detection events fire for all chats; enforcement is groups-only
            }
            if (markAntiBotSeen(message.key?.id)) {
                return; // already handled (message matched both bot + link detectors)
            }
            const settings = antiBotSettings.get(groupJid);
            if (!settings?.enabled) {
                return;
            }
            // Detection itself (isBotMessage/isLinkMessage, set upstream) already applies
            // to every message regardless of sender — including our own. We still never
            // take enforcement action (delete/kick) against our own messages below: we
            // can't remove ourselves, and it would just be noise.
            if (message.key.fromMe) {
                return;
            }
            const sender = message.key.participant ? jidNormalizedUser(message.key.participant) : undefined;
            if (!sender) {
                return;
            }
            let metadata;
            try {
                ...
                logger?.warn?.({ err, groupJid }, 'antibot: failed to fetch group metadata');
                return;
            }
            const senderParticipant = metadata.participants?.find(p => areJidsSameUser(p.id, sender));
            const senderIsAdmin = !!senderParticipant?.admin;
            const senderIsOwner = areJidsSameUser(sender, authState.creds.me?.id);
            if (senderIsAdmin || senderIsOwner) {
                return; // never act on admins/owner
            }
            const botParticipant = metadata.participants?.find(p => areJidsSameUser(p.id, authState.creds.me?.id));
            if (!botParticipant?.admin) {
                return; // bot must be admin to delete/remove
            }
            // best-effort delete of the offending message
            try {
                await sock.sendMessage(groupJid, { delete: message.key });
            }
            catch (err) {
                logger?.warn?.({ err, groupJid }, 'antibot: failed to delete message');
            }
            const mode = settings.mode || 'kick';
            if (mode === 'delete' || mode === 'null') {
                return; // silent — deletion already done above
            }
            if (mode === 'warn') {
                const warnKey = `${groupJid}:${sender}`;
                const warns = (antiBotWarns.get(warnKey) || 0) + 1;
                antiBotWarns.set(warnKey, warns);
                if (warns >= 3) {
                    await sock.groupParticipantsUpdate(groupJid, [sender], 'remove').catch(err => logger?.warn?.({ err, groupJid, sender }, 'antibot: warn-remove failed'));
                    antiBotWarns.delete(warnKey);
                }
                else {
                    await sock.sendMessage(groupJid, {
                        text: `⚠️ AntiBot Detected\n\n👤 User: @${sender.split('@')[0]}\n📌 Warn: ${warns}/3`,
                        mentions: [sender]
                    }).catch(err => logger?.warn?.({ err, groupJid, sender }, 'antibot: warn-message failed'));
                }
                return;
            }
            // default: kick
            await sock.groupParticipantsUpdate(groupJid, [sender], 'remove').catch(err => logger?.warn?.({ err, groupJid, sender }, 'antibot: kick failed'));
        }
        catch (err) {
            logger?.error?.({ err }, 'antibot: enforcement error');
        }
    };
    ev.on('messages.bot-detect', async (updates) => {
        for (const { message } of updates) {
            await handleAntiBotDetection(message);
        }
    });
    ev.on('messages.link-detect', async (updates) => {
        for (const { message } of updates) {
            await handleAntiBotDetection(message);
        }
    });
