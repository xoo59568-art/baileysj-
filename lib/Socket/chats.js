...
import { chatModificationToAppPatch, decodePatches, decodeSyncdSnapshot, encodeSyncdPatch, ensureLTHashStateVersion, extractSyncdPatches, generateFullProfilePicture, generateProfilePicture, getContentType, getHistoryMsg, isAppStateSyncIrrecoverable, isMissingKeyError, MAX_SYNC_ATTEMPTS, newLTHashState, processSyncAction } from '../Utils/index.js';
...
export const makeChatsSocket = (config) => {
     ...
     * a group's DP, or (if permitted) a contact's DP. Same jid rule as
     * updateProfilePicture: pass your own id, a group jid, or another jid.
     */
    const updateFullProfilePicture = async (jid, content, dimensions) => {
        let targetJid;
        if (!jid) {
            throw new Boom('Illegal no-jid profile update. Please specify either your ID or the ID of the chat you wish to update');
        }
        if (jidNormalizedUser(jid) !== jidNormalizedUser(authState.creds.me.id)) {
            targetJid = jidNormalizedUser(jid); // in case it is someone other than us (e.g. a group)
        }
        else {
            targetJid = undefined;
        }
        const { img } = await generateFullProfilePicture(content, dimensions);
        await query({
            tag: 'iq',
            attrs: {
                to: S_WHATSAPP_NET,
    ...
    /** alias kept for convenience — same function, works for groups and inbox chats alike */
    const updateFullGroupProfilePicture = updateFullProfilePicture;
    ...
    const upsertMessage = ev.createBufferedFunction(async (msg, type) => {
        ...
        if (!msg.key.fromMe && chatJid?.endsWith('@g.us') && getContentType(msg.message) === 'groupStatusMentionMessage') {
            ev.emit('group-status-mention', {
                chat: chatJid,
    ...
    return {
        ...
        updateFullProfilePicture,
        updateFullGroupProfilePicture,
