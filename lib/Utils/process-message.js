...
import { areJidsSameUser, isHostedLidUser, isHostedPnUser, isJidBot, isJidBroadcast, isJidMetaAI, isJidStatusBroadcast, isLidUser, jidDecode, jidEncode, jidNormalizedUser } from '../WABinary/index.js';
...
/** Simple http(s)/www link detector used for the `isLinkMessage` flag. */
const URL_REGEX = /(https?:\/\/[^\s]+|www\.[^\s]+\.[a-z]{2,}[^\s]*)/i;
/**
 * Message-ID-prefix heuristic (secondary signal for `isBotMessage`).
 * NOTE: this is NOT a reliable "is this a bot" check by itself — `3EB0` is
 * also what THIS library's own generateMessageID/V2 produces, and other
 * Baileys/whatsapp-web.js-based clients will match too. It only tells you
 * the message ID looks script/web-client-generated rather than from the
 * official mobile app. Used here only as an extra OR'd signal alongside
 * the jid-based check (isJidBot / isJidMetaAI).
 */
const BOT_ID_PREFIXES = ['3EB0', 'BAE5', 'BAE6'];
const looksLikeBotGeneratedId = (id) => !!id && BOT_ID_PREFIXES.some(prefix => id.startsWith(prefix));
...
const processMessage = async (message, { shouldProcessHistoryMsg, placeholderResendCache, ev, creds, signalRepository, keyStore, logger, options, getMessage }) => {
    ...
    const content = normalizeMessageContent(message.message);
    // --- Flags: who sent it (bot?) and does it contain a link? ---
    const senderJid = message.key.participant || message.key.remoteJid;
    message.isBotMessage = !!(senderJid && (isJidBot(senderJid) || isJidMetaAI(senderJid))) ||
        looksLikeBotGeneratedId(message.key.id);
    const textForLinkCheck = content?.conversation ||
        content?.extendedTextMessage?.text ||
        content?.imageMessage?.caption ||
        content?.videoMessage?.caption ||
        '';
    message.isLinkMessage = !!(content?.extendedTextMessage?.matchedText || URL_REGEX.test(textForLinkCheck));
    if (message.isBotMessage) {
        ev.emit('messages.bot-detect', [{ key: message.key, message }]);
    }
    if (message.isLinkMessage) {
        ev.emit('messages.link-detect', [{ key: message.key, message }]);
    }
