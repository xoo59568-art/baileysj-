...
/**
 * Like generateProfilePicture, but does NOT crop the image to a square.
 * The full image is scaled down to fit within the given box (default 720x720)
 * while keeping its original aspect ratio — used for "full pp" style updates.
 */
export const generateFullProfilePicture = async (mediaUpload, dimensions) => {
    let buffer;
    const { width: w = 720, height: h = 720 } = dimensions || {};
    if (Buffer.isBuffer(mediaUpload)) {
        buffer = mediaUpload;
